...
Después crea un trigger de sustitución que gestione las operaciones de actualización, inserción y borrado 
del siguiente modo:
c.	Se insertan datos solamente en la tabla emple, de tal modo que cuando se detecta un insert toma los 
datos de la sentencia e inserta en la tabla emple los datos necesarios (Emp_no, Apellido, Oficio, Salario, 
ept_no)
d.	Se actualizan datos solamente de la tabla emple. Si se detecta una actualización  sólo actualiza la 
información de la tabla emple, pudiendo cambiar al empleado de departamento.
e.	Se eliminan datos de la tabla emple. Si existe una eliminación de registros  se eliminan los registros
de emple, pero no de Depart. (Analiza que sucede si eliminamos el/los registro/s de Depart.
*/
...
c.	Se insertan datos en la tabla pedidos (no dejando insertar artículos nuevos), con una fecha_pedido, 
el nif de la tienda y unidades_pedidas.
...
Ahora crea un trigger que administre las modificaciones sobre los datos mostrados de esta vista del 
siguiente modo:
d.	Se insertan datos en la tabla Ventas (se crean nuevas ventas).
e.	Si se inserta un articulo que no existe 
1.	Se da de alta el articulo (articulo, cod_fabricante, peso, categoría) en la tabla Artículos (Obs: el 
fabricante debe existir previamente)
...
g.	Si eliminamos datos, igualmente sólo de la tabla Ventas. De este modo estamos eliminando la venta que 
pasamos en la orden DELETE y no otra
1.	Observación: No servirá solo con pasar en la orden DELETE la clave ajena de la tabla Ventas.
*/

/* 
4.	Crea un paquete que gestione todas las operaciones sobre la tabla CENTROS con los siguientes 
procedimientos:
a.	INSERTAR_CENTRO: Controla que no se repite el nº de centro cuando pasamos el nº  de centro y el 
nombre como parámetro. Además controla que siempre se inserta en mayúsculas el nombre.
b.	BORRAR_CENTRO: elimina el centro que pasamos como parámetro, pero 1º crea un centro auxiliar del 
formato: AUX_NOMBRE_CENTRO_ELIMINADO, y con nº la decena mayor que se encuentre, y pasa aquí sus registros
 subordinados de cualquier tabla (Personal y Profesores). Después elimina el centro pasado como parámetro.
...
b.	ELIMINAR_ARTICULO: Elimina el articulo que pasamos como parámetro (articulo, cod_fabricante, peso, 
categoría), pero previamente copia este registro en la tabla H_ARTICULOS, y posteriormente pasa a las 
tablas H_PEDIDOS y H_VENTAS todos sus pedidos y ventas respectivamente. Una vez realizado todo esto podemos
eliminar el artículo.
c.	CREAR_ARTICULO: Pasamos todos los parámetros comprobando que son correctos y teniendo en cuenta 
incidencias del tipo como que no puede ser menor el precio de venta que el precio de coste.
d.	MODIFICAR_ARTICULO: Cambiamos sólo los campos no pertenecientes a la PK  precio venta, precio coste,
 unidades. Pasamos como parámetro la PK del articulo y los valores a cambiar.
*/


/* HACER
6.	Creamos un trigger que nos permite auditar la tabla ARTÍCULOS de tal modo que registramos en una tabla
 que operación se ha realizado con que artículo, quien la ha hecho y cuando. Esa tabla es:
a.	Auditar_articulos: campos: Articulo, cod_fabricante, peso, categoría, usuario, fecha, 
 tipo_modificación.
b.	Cuando insertamos registros en la tabla Artículos inmediatamente se crea un registro en 
...
d.	Cuando modificamos un campo del artículo  en el campo correspondiente de Auditar_Articulos se 
...
elimina ese mismo registro de la tabla NUEVOS si existe allí todavía.
*/
CREATE OR REPLACE TRIGGER COPIA_NUEVO
BEFORE INSERT ON NUEVOS
FOR EACH ROW -- Ó FOR EACH STATEMENT(CADA ORDEN, QUE NO SIRVE LO DE OLD O NEW)
BEGIN
...
/*
10.	 Crea un paquete que gestiona la tabla PERSONAS y PROVINCIAS 
(descripción de las tablas en la página 138)
a.	Cuando insertamos registros en la tabla PERSONAS se calcula el nº de personas en esa provincia y
 estos datos se insertan en la tabla, o se actualizan, dependiendo si ya existen personas en esa provincia.
1.	EST_PROVINCIAS: Cod_prov, nom_prov, num_per.
b.	Cuando eliminamos registros también actualizamos en esta tabla el campo num_per, quedando a 0 el 
campo num_per, pero no eliminamos el registro de la provincia en la tabla EST_PROVINCIAS.
c.	Cuando actualizamos en la tabla PERSONAS y cambiamos a alguien de provincia  se vuelven a calcular 
el número de personas en cada provincia y se actualiza en la tabla EST_PROVINCIAS.
*/
...
11.	Crea un trigger sobre la tabla PROFESORES  de tal modo que cuando se inserte, borre, o actualice un
 ...
 si se actualizan datos controla los cambios en la tabla ‘Personal’ sólo de los datos que tengan en común
  ambas tablas.
*/
...
registro de PERSONAL  se haga lo mismo en la tabla PROFESORES (Sólo si la acción recae sobre un profesor).
 ...
 sólo de los datos que tengan en común ambas tablas.
*/
